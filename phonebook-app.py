...
app.config['MYSQL_DATABASE_PASSWORD'] = 'XXXXXXXXXXXX' #Your db password
...
def find_records():
    if request.method == 'POST':
        ...
        return render_template('index.html', persons_html=persons_app, keyword=keyword, show_result=True, developer_name='XXXXXXXX') #Your Name
    else:
        return render_template('index.html', show_result=False, developer_name='XXXXXXXX') #Your Name
...
def add_record():
    if request.method == 'POST':
        ...
        if name is None or name.strip() == "":
            return render_template('add-update.html', not_valid=True, message='Invalid input: Name can not be empty', show_result=False, action_name='save', developer_name='XXXXXXXXX')
            ...
            return render_template('add-update.html', not_valid=True, message='Invalid input: Name of person should be text', show_result=False, action_name='save', developer_name='XXXXXXXXX')
        ...
        if phone_number is None or phone_number.strip() == "": # The user may have forgotten to give a number. This function will control whether the phone number is empty or not. if it is empty, a warning massage will be raising. 
            return render_template('add-update.html', not_valid=True, message='Invalid input: Phone number can not be empty', show_result=False, action_name='save', developer_name='XXXXXXXXX')
            ...
            return render_template('add-update.html', not_valid=True, message='Invalid input: Phone number should be in numeric format', show_result=False, action_name='save', developer_name='XXXXXXXXX')
        # if everything is ok, whole those blocks will be passed, and we come here. 
        result_app = insert_person(name, phone_number)
        return render_template('add-update.html', show_result=True, result_html=result_app, not_valid=False, action_name='save', developer_name='XXXXXXXXX') #In addition, There is no message shown by user here. Thats why not valid is going to be False.
    else:
        return render_template('add-update.html', show_result=False, not_valid=False, action_name='save', developer_name='XXXXXXXXX')  #Your Name
...
def update_record():
    if request.method == 'POST':
        ...
        if name is None or name.strip() == "":
            return render_template('add-update.html', not_valid=True, message='Invalid input: Name can not be empty', show_result=False, action_name='update', developer_name='XXXXXXXXX') #Your Name
        phone_number = request.form['phonenumber']
        if phone_number is None or phone_number.strip() == "":
            return render_template('add-update.html', not_valid=True, message='Invalid input: Phone number can not be empty', show_result=False, action_name='update', developer_name='XXXXXXXX') #Your Name
        elif not phone_number.isdecimal():
            return render_template('add-update.html', not_valid=True, message='Invalid input: Phone number should be in numeric format', show_result=False, action_name='update', developer_name='XXXXXXXX') #Your Name

        result_app = update_person(name, phone_number) #
        return render_template('add-update.html', show_result=True, result_html=result_app, not_valid=False, action_name='update', developer_name='XXXXXXXXXX') #Again, There is no message shown by user here. Thats why not valid is going to be False.
    else:
        return render_template('add-update.html', show_result=False, not_valid=False, action_name='update', developer_name='XXXXXXXXX')  #Your Name
...
def delete_record():
    if request.method == 'POST':
        name = request.form['username']
        if name is None or name.strip() == "":
            return render_template('delete.html', not_valid=True, message='Invalid input: Name can not be empty', show_result=False, developer_name='XXXXXXXXX') #Your Name
        result_app = delete_person(name)
        return render_template('delete.html', show_result=True, result_html=result_app, not_valid=False, developer_name='XXXXXXXXX') # In addition, There will be no message to be shown to the user here. Thats why not valid is going to be False.
    else:
        return render_template('delete.html', show_result=False, not_valid=False, developer_name='XXXXXXXXX')  #Your Name
